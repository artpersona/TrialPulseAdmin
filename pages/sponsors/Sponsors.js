...
export default function Sponsors() {
	...
	return (
		<div className="min-h-screen flex">
			<sidebar className="min-h-screen w-4/12 border-r border-r-gray-light flex flex-col">
				<section className="flex w-full justify-center h-[70px] bg-gray-primary">
					<article className="flex w-[400px]">
						{a.map((item, index) => {
							return (
								<p key={index} className="hover:bg-gray-secondary text-white py-4 px-12 cursor-pointer text-[14px]">
									{item}
								</p>
							);
						})}
								...
								<>
									...
									<div className="w-full rounded-[2.2rem] border-2 border-gray-secondary mt-4 p-[0.65rem] shadow-md cursor-pointer space-y-3">
										{c.map((item, index) => {
											return (
												<>
													<figure key={index} className="flex px-2 items-center justify-between">
							...
							{d.length !== 0 ? (
								<>
									{Object.entries(dict).map((item, index) => {
										return (
											<>
												<figure key={index} className="bg-gray-light flex items-center space-x-2 rounded-full pl-4 font-medium text-gray-primary mt-4">
													<p>{item[0]}</p>
												</figure>
												<div key={item[0]} className="w-full rounded-[2.2rem] border-2 border-gray-secondary mt-4 p-[0.65rem] shadow-md cursor-pointer space-y-3">
			...
			<main className="h-full w-8/12 bg-white">
